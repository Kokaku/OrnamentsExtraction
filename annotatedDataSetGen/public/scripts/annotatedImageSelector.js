...
function editRectanglePosition(originalRectangle, newRectangle) {
    return {
        x: newRectangle.x,
        y: newRectangle.y,
        w: newRectangle.w,
        h: newRectangle.h,
        nature: originalRectangle.nature,
        type: originalRectangle.type,
        color: 'color' in originalRectangle ? originalRectangle.color : "green"
    }
}
...
function editRectangle(mousePos) {
    ...
    switch(selectedRect.t) {
        case "top":
        case "bottom":
            rectangles[selectedRect.r] = editRectanglePosition(rectangles[selectedRect.r],
                getRectangle(selectedRectPos, {
                    x: rectangles[selectedRect.r].x + currentImagePosition.x + rectangles[selectedRect.r].w,
                    y: mousePos.y
                }));
            break;
        ...
        default:
            rectangles[selectedRect.r] = editRectanglePosition(rectangles[selectedRect.r],
                getRectangle(selectedRectPos, mousePos));
    }
}
...
function setSelectedRectangle(mousePos) {
    ...
    if(selectedRect.r >= 0 && 'iou' in pages[selectedPage].rectangles[selectedRect.r]) {
        console.log(pages[selectedPage].rectangles[selectedRect.r].iou);
    }
    enableSelectTagOrnament();
...
function draw() {
    ...
    for (var i = 0; i < pages[selectedPage].rectangles.length; i++) {
        ...
        if(i == selectedRect.r) {
            ...
            context.strokeStyle = 'color' in rect ? rect.color : "green";
...
function loadPages(browserPosition, limit, pushSide, pageToShow, callback) {
    ...
    var queryUrl = document.getElementById("displayScores").checked ? "/scores" : "/annotatedPages";
    queryDB(queryUrl, params, function(result) {
        if (result.length > 0) {
...
function parseOrnaments(strOrnaments) {
    ...
    for (var i=0; i<strOrnaments.length; i++) {
        ornament = {
            ...
            color: strOrnaments[i].color
        };
        if('iou' in strOrnaments[i]) {
            ornament.iou =  strOrnaments[i].iou;
        }
...
function displayScores() {
    areScoresDisplayed = document.getElementById("displayScores").checked;
    document.getElementById("ornamentsFilter").disabled = areScoresDisplayed;

    if(areScoresDisplayed) {
        statMemory.newRandomPage = document.getElementById("newRandomPage").disabled;
        document.getElementById("newRandomPage").disabled = false;
    } else {
        console.log(statMemory.newRandomPage)
        document.getElementById("newRandomPage").disabled = statMemory.newRandomPage;
    }
