...
app.listen(3000, function() {
    ...
    var fs = require("fs");
    var jsonContent = JSON.parse(fs.readFileSync("public/scores.json"));
    scores = jsonContent.evaluations.map(function(element) {
        var ornaments = [];
        for (var i=0; i<element.results.length; i++) {
            if(element.results[i].trueBox != null) {
                ornaments.push({
                    "x": element.results[i].trueBox.x,
                    "y": element.results[i].trueBox.y,
                    "w": element.results[i].trueBox.w,
                    "h": element.results[i].trueBox.h,
                    "nature": 'Unknown',
                    "type": 'Unknown',
                    "color": 'green'
                });
            ...
            if(element.results[i].proposal != null) {
                ornaments.push({
                    "x": element.results[i].proposal.x,
                    "y": element.results[i].proposal.y,
                    "w": element.results[i].proposal.w,
                    "h": element.results[i].proposal.h,
                    "nature": 'Unknown',
                    "type": 'Unknown',
                    "color": 'GoldenRod',
                    "iou": element.results[i].iou
                });
            }
        }
        return {
            '_id': 'http://dhlabsrv4.epfl.ch/iiif_ornaments/bookm-'+element.bookId+'_'+element.pageId+'/full/full/0/default.jpg',
            'ornaments': ornaments
        }
    });
});
...
app.post('/scores', function(req, res) {
    requestedScore = scores.filter(function(elem) {
        return elem.ornaments.length > 0;
    });

    var limit = Math.min(requestedScore.length, parseInt(req.body.limit));
    var position;
    if(req.body.position == 'Infinity') {
        position = requestedScore.length - limit;
    } else {
        position = parseInt(req.body.position);
    }

    res.send({
        res: requestedScore.slice(position, position+limit),
        position: position+limit-1,
        annotatedPagesCount: requestedScore.length
    });
});
